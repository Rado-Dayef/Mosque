...
import 'package:com.rado.mosque/views/widgets/build_default_dialog.dart';
...
class SebhaController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  Future<void> deleteData(int i) async {
    int res = await sqlDb.deleteData("DELETE FROM 'sebha' WHERE id = ${sebhaList[i][AppConstants.idText]}");
    if (res > 0) {
      sebhaList.removeWhere((element) => element[AppConstants.idText] == sebhaList[i][AppConstants.idText],
      );
    }
  }

  Future<void> showDeleteDialog(int i) async {
    i == -1 ? await Get.dialog(
      Directionality(
